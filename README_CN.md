...
- **(2026/5/22)** 部署与仿真启动链路升级 — 新增更清晰的分阶段启动流程：Docker 镜像拉取式快速体验、Phase 1 Mock Runtime、Phase 2 LLM 自主控制链路、Phase 3 PX4+Gazebo 完整仿真。新的 `sim_quickstart.sh` 支持一键编排 PX4 SITL、Gazebo、AerialClaw backend、Web UI、Gazebo GUI、MAVSDK 控制链路与 camera/LiDAR sensor bridge，并加入 `/api/adapter/status`、`/api/sensor/status`、`/api/sensor/camera` 确定性健康检查。该版本同时禁止 PX4 控制失败后静默降级到 mock，固定仿真关键 Python 依赖，并默认关闭/清理 PX4 ULog 持久日志，保证本地演示环境可复现、可诊断且不会异常占满磁盘。
...
真机迁移请参阅：[真机迁移与室内试飞指南](docs/REAL_FLIGHT_MIGRATION_CN.md)
